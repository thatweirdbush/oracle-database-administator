...
--      EXECUTE N09_ADMIN.N09_SELECT_ANY_TABLE(:rc, 'N09_ADMIN.UV_N09_DONVI');
--  PRINT rc;
--/
...
--  CONN NV201/NV201
--  VARIABLE rc REFCURSOR;
--      EXECUTE N09_ADMIN.N09_SELECT_ANY_TABLE(:rc, 'N09_ADMIN.UV_N09_DONVI', '1=0');
--  PRINT rc;
--/
...
--CONN NV201/NV201;
--VARIABLE rc REFCURSOR;
--     EXECUTE N09_ADMIN.N09_GET_SINGLE_LINE_DATA(:rc, 'N09_ADMIN.N09_NHANSU');
--PRINT rc;
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_NHANSU SET ' || STR_COLUMN || ' = ''' || STR_VALUE || ''' WHERE MANV = ''' || STR_ID || '''';
END;
/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_SINHVIEN SET ' || STR_COLUMN || ' = ''' || STR_VALUE || ''' WHERE MASV = ''' || STR_ID || '''';
END;
/
...
BEGIN
    sql_query := 'SELECT D."Mã Sinh Viên", D."Mã Giảng Viên", D."Mã Học Phần", D."Học Kỳ", D."Năm", D."Mã Chương Trình",
        ...
        FROM N09_ADMIN.UV_N09_DANGKY D, N09_ADMIN.UV_N09_PHANCONG P 
...
--CONN NV201/NV201;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_DANGKY_JOIN_PHANCONG_BY_GIANGVIEN(:rc, 'NV201', 'HP001', 1, 2024, 'CQ');
--PRINT rc;
--/   
...
--CONN NV201/NV201;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_GET_CURRENT_USER_ROLE(:rc);
--PRINT rc;
--/   
...
AS
BEGIN
    OPEN OUTPUT FOR 'SELECT COUNT(*) FROM N09_ADMIN.N09_SINHVIEN WHERE MASV = ''' || STR_MASV || '''';
END;
/
...
-- CONN NV301/NV301;
-- VARIABLE rc REFCURSOR;
-- EXECUTE N09_ADMIN.N09_IS_EXIST_SINHVIEN(:rc, 'SV001');
-- PRINT rc;
-- /
...
AS
BEGIN
    OPEN OUTPUT FOR 'SELECT COUNT(*) FROM N09_ADMIN.N09_NHANSU WHERE MANV = ''' || STR_MANV || '''';
END;
/
...
--CONN NV301/NV301;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_IS_EXIST_NHANSU(:rc, 'NV301');
--PRINT rc;
--/
...
AS
BEGIN
    OPEN OUTPUT FOR 'SELECT COUNT(*) FROM N09_ADMIN.N09_DONVI WHERE MADV = ''' || STR_MADV || '''';
END;
/
...
--CONN NV301/NV301;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_IS_EXIST_DONVI(:rc, 'KHOA1');
--PRINT rc;
--/
...
AS
BEGIN
    OPEN OUTPUT FOR 'SELECT COUNT(*) FROM N09_ADMIN.N09_HOCPHAN WHERE MAHP = ''' || STR_MAHP || '''';
END;
/
...
--CONN NV301/NV301;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_IS_EXIST_HOCPHAN(:rc, 'HP001');
--PRINT rc;
--/
...
AS
BEGIN
    OPEN OUTPUT FOR 'SELECT COUNT(*) FROM N09_ADMIN.N09_KHMO WHERE MAHP = ''' || STR_MAHP || ''' AND HK = ' || STR_HK || ' AND NAM = ' || STR_NAM || ' AND MACT = ''' || STR_MACT || '''';
END;
/
...
--CONN NV301/NV301;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_IS_EXIST_KHMO(:rc, 'HP001', 1, 2024, 'CQ');
--PRINT rc;
--/
...
AS
BEGIN
    OPEN OUTPUT FOR 'SELECT COUNT(*) FROM N09_ADMIN.N09_DANGKY WHERE MASV = ''' || STR_MASV || ''' AND MAGV = ''' || STR_MAGV || ''' AND MAHP = ''' || STR_MAHP || ''' AND HK = ' || STR_HK || ' AND NAM = ' || STR_NAM || ' AND MACT = ''' || STR_MACT || '''';
END;
/
...
--CONN NV301/NV301;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_IS_EXIST_DANGKY(:rc, 'SV001', 'NV301', 'HP001', 1, 2024, 'CQ');
--PRINT rc;
--/
...
AS
BEGIN
    OPEN OUTPUT FOR 'SELECT COUNT(*) FROM N09_ADMIN.N09_PHANCONG WHERE MAGV = ''' || STR_MAGV || ''' AND MAHP = ''' || STR_MAHP || ''' AND HK = ' || STR_HK || ' AND NAM = ' || STR_NAM || ' AND MACT = ''' || STR_MACT || '''';
END;
/
...
--CONN NV301/NV301;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_IS_EXIST_PHANCONG(:rc, 'NV301', 'HP001', 1, 2024, 'CQ');
--PRINT rc;
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'INSERT INTO N09_ADMIN.N09_SINHVIEN(MASV, HOTEN, PHAI, NGSINH, DIACHI, DT, MACT, MANGANH, SOTCTL, DTBTL, COSO) 
...
--EXECUTE N09_ADMIN.N09_INSERT_SINHVIEN('SV201', 'Nguyen Van A', 'Nam', TO_DATE('01/01/2000', 'DD/MM/YYYY'), 'Ha Noi', '0123456789', 'CT1', 'CN1', 120, 8.5, 'CS1');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_SINHVIEN SET HOTEN = :1, PHAI = :2, NGSINH = :3, DIACHI = :4, DT = :5, MACT = :6, MANGANH = :7, SOTCTL = :8, DTBTL = :9, COSO = :10 WHERE MASV = :11'
...
-- CONN NV301/NV301;
-- SELECT * FROM N09_ADMIN.N09_SINHVIEN;
-- EXECUTE N09_ADMIN.N09_UPDATE_SINHVIEN('SV001', 'Nguyen Van B', 'Nam', TO_DATE('01/01/2000', 'DD/MM/YYYY'), 'Ha Noi', '0123456789', 'CT1', 'CN1', 120, 8.5, 'CS1');
-- /
...
IS
BEGIN
    EXECUTE IMMEDIATE 'INSERT INTO N09_ADMIN.N09_DANGKY(MASV, MAGV, MAHP, HK, NAM, MACT, DIEMTH, DIEMQT, DIEMCK, DIEMTK) 
...
--EXECUTE N09_ADMIN.N09_INSERT_DANGKY('SV001', 'NV201', 'HP001', 1, 2024, 'CQ', 10, 10, 10, 10);
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_DANGKY SET DIEMTH = :1, DIEMQT = :2, DIEMCK = :3, DIEMTK = :4 WHERE MASV = :5 AND MAGV = :6 AND MAHP = :7 AND HK = :8 AND NAM = :9 AND MACT = :10'
...
--SELECT * FROM N09_ADMIN.N09_DANGKY;
--VARIABLE rc REFCURSOR;
--EXECUTE N09_ADMIN.N09_SELECT_ANY_TABLE(:rc, 'N09_ADMIN.N09_DANGKY');
--PRINT rc;
--EXECUTE N09_ADMIN.N09_UPDATE_DANGKY('SV001', 'NV201', 'HP001', 1, 2024, 'CQ', 10, 10, 10, 10);
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'INSERT INTO N09_ADMIN.N09_PHANCONG(MAGV, MAHP, HK, NAM, MACT) VALUES(:1, :2, :3, :4, :5)'
...
--EXECUTE N09_ADMIN.N09_INSERT_PHANCONG('NV201', 'HP001', 1, 2024, 'CQ');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_PHANCONG SET MAGV = :1, MAHP = :2, HK = :3, NAM = :4, MACT = :5 WHERE MAGV = :6 AND MAHP = :7 AND HK = :8 AND NAM = :9 AND MACT = :10'
...
---- Test
--CONN NV301/NV301;
--SELECT * FROM N09_ADMIN.N09_PHANCONG WHERE MAHP = 'HP032';
--/
--
---- UPDATE Học phần HP006 không thuộc Văn phòng khoa: Không thành công
--CONN NV301/NV301;
--EXECUTE N09_ADMIN.N09_UPDATE_PHANCONG('NV217', 'HP006', 3, 2077, 'CLC', 'NV217', 'HP006', 3, 2024, 'CLC');
--/
--
---- UPDATE Học phần HP032 thuộc Văn phòng khoa: Thành công
--CONN NV301/NV301;
--EXECUTE N09_ADMIN.N09_UPDATE_PHANCONG('NV209', 'HP032', 2, 2025, 'CQ', 'NV209', 'HP032', 2, 2024, 'CQ');
--/
--
---- UPDATE Học phần HP032 thuộc Văn phòng khoa: Thành công
--CONN NV301/NV301;
--EXECUTE N09_ADMIN.N09_UPDATE_PHANCONG('NV209', 'HP032', 2, 2024, 'CQ', 'NV209', 'HP032', 2, 2025, 'CQ');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'INSERT INTO N09_ADMIN.N09_DONVI(MADV, TENDV, TRGDV) VALUES(:1, :2, :3)'
...
--EXECUTE N09_ADMIN.N09_INSERT_DONVI('DV008', 'Bộ môn TKVM', 'NV103');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_DONVI SET TENDV = :1, TRGDV = :2 WHERE MADV = :3'
...
--CONN NV301/NV301;
--SELECT * FROM N09_ADMIN.N09_DONVI;
--EXECUTE N09_ADMIN.N09_UPDATE_DONVI('DV001', 'Khoa CNTT', 'Nguyen Van A');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'INSERT INTO N09_ADMIN.N09_HOCPHAN(MAHP, TENHP, SOTC, STLT, STTH, SOSVTD, MADV) VALUES(:1, :2, :3, :4, :5, :6, :7)'
...
---- Test
--CONN NV301/NV301;
--SELECT * FROM N09_ADMIN.N09_HOCPHAN;
--EXECUTE N09_ADMIN.N09_INSERT_HOCPHAN('HP008', 'Lập trình hướng đối tượng', 3, 2, 1, 50, 'DV001');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_HOCPHAN SET TENHP = :1, SOTC = :2, STLT = :3, STTH = :4, SOSVTD = :5, MADV = :6 WHERE MAHP = :7'
...
--CONN NV301/NV301;
--SELECT * FROM N09_ADMIN.N09_HOCPHAN;
--EXECUTE N09_ADMIN.N09_UPDATE_HOCPHAN('HP008', 'Lập trình hướng đối tượng', 3, 2, 1, 50, 'DV001');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'INSERT INTO N09_ADMIN.N09_KHMO(MAHP, HK, NAM, MACT) VALUES(:1, :2, :3, :4)'
...
--EXECUTE N09_ADMIN.N09_INSERT_KHMO('HP008', 1, 2024, 'CQ');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_KHMO SET HK = :1, NAM = :2, MACT = :3, MAHP = :4 WHERE MAHP = :5 AND HK = :6 AND NAM = :7 AND MACT = :8'
...
--CONN NV301/NV301;
--SELECT * FROM N09_ADMIN.N09_KHMO;
--EXECUTE N09_ADMIN.N09_UPDATE_KHMO('HP034', 6, 9009, 'CLC', 'HP034', 6, 9999, 'CLC');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'INSERT INTO N09_ADMIN.N09_NHANSU(MANV, HOTEN, PHAI, NGSINH, PHUCAP, DT, VAITRO, MADV, COSO) 
...
--CONN NV001/NV001;
--SELECT * FROM N09_ADMIN.N09_NHANSU;
--EXECUTE N09_ADMIN.N09_INSERT_NHANSU('NV201', 'Nguyen Van A', 'Nam', TO_DATE('01/01/2000', 'DD/MM/YYYY'), 1000000, '0123456789', 'GV', 'DV001', 'CS1');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'UPDATE N09_ADMIN.N09_NHANSU SET HOTEN = :1, PHAI = :2, NGSINH = :3, PHUCAP = :4, DT = :5, VAITRO = :6, MADV = :7, COSO = :8 WHERE MANV = :9'
...
--CONN NV301/NV301;
--SELECT * FROM N09_ADMIN.N09_NHANSU;
--EXECUTE N09_ADMIN.N09_UPDATE_NHANSU('NV201', 'Nguyen Van A', 'Nam', TO_DATE('01/01/2000', 'DD/MM/YYYY'), 1000000, '0123456789', 'GV', 'DV001', 'CS1');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'DELETE FROM N09_ADMIN.N09_DANGKY WHERE MASV = :1 AND MAGV = :2 AND MAHP = :3 AND HK = :4 AND NAM = :5 AND MACT = :6'
...
--CONN NV301/NV301;
--SELECT * FROM N09_ADMIN.N09_DANGKY;
--EXECUTE N09_ADMIN.N09_DELETE_DANGKY('SV001', 'NV201', 'HP001', 1, 2024, 'CQ');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'DELETE FROM N09_ADMIN.N09_NHANSU WHERE MANV = :1'
                        USING STR_MANV;
END;
/
...
--CONN NV001/NV001;
--SELECT * FROM N09_ADMIN.N09_NHANSU;
--EXECUTE N09_ADMIN.N09_DELETE_NHANSU('NV201');
--/
...
IS
BEGIN
    EXECUTE IMMEDIATE 'DELETE FROM N09_ADMIN.N09_PHANCONG WHERE MAGV = :1 AND MAHP = :2 AND HK = :3 AND NAM = :4 AND MACT = :5'
                        USING STR_MAGV, STR_MAHP, STR_HK, STR_NAM, STR_MACT;
END;
/
...
--CONN NV301/NV301;
--SELECT * FROM N09_ADMIN.N09_PHANCONG;
--EXECUTE N09_ADMIN.N09_DELETE_PHANCONG('NV201', 'HP001', 1, 2024, 'CQ');
--/
...
BEGIN
    FOR REC IN CUR LOOP
        USR := REC.MANV; 
        STRSQL := 'GRANT ' || STR_PRIV || ' ON N09_ADMIN.N09_THONGBAO TO ' || USR; 
        EXECUTE IMMEDIATE STRSQL; 
    END LOOP; 
END;
/
...
/***************************************************************
* Back to PH1 - Create Test User For Role Testing
****************************************************************/
-- Create User
CONN N09_ADMIN/123@//localhost:1521/PDB_N09;
